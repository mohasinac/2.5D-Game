  ...
  return (
    <div className="p-6 max-w-[800px]">
      <div className="flex items-start justify-between mb-5">
        <div>
          <h1 className="text-[22px] font-bold text-text m-0">Trigger Type Defs</h1>
          <p className="text-[13px] text-muted mt-1">
            Defines trigger condition types for part stat modifiers. Falls back to built-ins when empty.
          </p>
        </div>
        <Button variant="primary" size="sm" onClick={openCreate}>+ Add Trigger Type</Button>
      </div>
      ...
      {loading ? (
        ...
        <div className="text-muted text-[13px]">No trigger types found. Add one above.</div>
      ) : (
      ...
      {showModal && (
        <div className="fixed inset-0 bg-black/75 flex items-center justify-center z-[1000]">
          <div className="bg-bg0 border border-border rounded-2xl p-7 w-[420px] max-w-[90vw]">
            <h2 className="text-[17px] font-bold text-text m-0 mb-5">{editing ? "Edit" : "Add"} Trigger Type</h2>
            <div className="mb-3">
              <Label>ID (slug) *</Label>
              <Input value={form.id} onChange={e => setForm(f => ({ ...f, id: e.target.value }))} disabled={!!editing} className={editing ? "opacity-50" : ""} placeholder="on_hit" />
            </div>
            <div className="mb-3">
              <Label>Label *</Label>
              <Input value={form.label} onChange={e => setForm(f => ({ ...f, label: e.target.value }))} placeholder="On Hit" />
            </div>
            <div className="mb-5">
      ...
      {confirmDelete && (
        <div className="fixed inset-0 bg-black/75 flex items-center justify-center z-[1000]">
          <div className="bg-bg0 border border-border rounded-2xl p-7 w-[360px] text-center">
            ...
            <div className="text-[13px] text-muted mb-5">This cannot be undone. Stat modifiers using this trigger type will fall back to displaying the raw ID.</div>
