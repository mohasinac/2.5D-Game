...
interface TipShapeDoc { id: string; label: string; description?: string; }

const EMPTY: TipShapeDoc = { id: "", label: "", description: "" };
...
export function TipShapeDefsPage() {
  const [items, setItems] = useState<TipShapeDoc[]>([]);
  const [loading, setLoading] = useState(true);
  const [showModal, setShowModal] = useState(false);
  const [editing, setEditing] = useState<TipShapeDoc | null>(null);
  const [form, setForm] = useState<TipShapeDoc>({ ...EMPTY });
  const [confirmDelete, setConfirmDelete] = useState<TipShapeDoc | null>(null);
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      setItems(docs);
    } catch { toast.error("Failed to load tip shapes"); }
  ...
  const openEdit = (item: TipShapeDoc) => { setEditing(item); setForm({ ...item }); setShowModal(true); };
  ...
  const handleSave = async () => {
    ...
    try {
      const { id, ...data } = form;
      await setDoc(doc(db, COLLECTIONS.TIP_SHAPE_DEFS, id.trim()), { ...data, updatedAt: new Date().toISOString() }, { merge: true });
      toast.success(editing ? "Updated" : "Created");
      invalidate("tipShapes");
      setShowModal(false);
      load();
  ...
  const handleDelete = async () => {
    ...
    try {
      await deleteDoc(doc(db, COLLECTIONS.TIP_SHAPE_DEFS, confirmDelete.id));
  ...
  return (
    <div style={{ padding: 24, maxWidth: 800 }}>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 20 }}>
        <div>
          <h1 style={{ fontSize: 22, fontWeight: 700, color: C.text, margin: 0 }}>Tip Shape Defs</h1>
          <p style={{ fontSize: 13, color: C.muted, margin: "4px 0 0" }}>
            Defines the tip shapes available in the part editor. Falls back to built-ins when empty.
          </p>
        </div>
      ...
      {loading ? (
        ...
        <div style={{ color: C.muted, fontSize: 13 }}>No tip shapes found. Add one above or seed the collection.</div>
      ) : (
      ...
      {showModal && (
        <div style={{ position: "fixed", inset: 0, background: "#00000088", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 1000 }}>
          <div style={{ background: C.bg0, border: `1px solid ${C.border}`, borderRadius: 14, padding: 28, width: 420, maxWidth: "90vw" }}>
            <h2 style={{ fontSize: 17, fontWeight: 700, color: C.text, margin: "0 0 20px" }}>{editing ? "Edit" : "Add"} Tip Shape</h2>
            <label style={{ display: "block", marginBottom: 12 }}>
              <span style={{ fontSize: 12, color: C.muted, display: "block", marginBottom: 4 }}>ID (slug, e.g. "rubber_flat") *</span>
              <input value={form.id} onChange={e => setForm(f => ({ ...f, id: e.target.value }))} disabled={!!editing} style={inp} placeholder="rubber_flat" />
            </label>
            <label style={{ display: "block", marginBottom: 12 }}>
              <span style={{ fontSize: 12, color: C.muted, display: "block", marginBottom: 4 }}>Label (display name) *</span>
              <input value={form.label} onChange={e => setForm(f => ({ ...f, label: e.target.value }))} style={inp} placeholder="Rubber Flat" />
            </label>
      ...
      {confirmDelete && (
        <div style={{ position: "fixed", inset: 0, background: "#00000088", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 1000 }}>
          <div style={{ background: C.bg0, border: `1px solid ${C.border}`, borderRadius: 14, padding: 28, width: 360, textAlign: "center" }}>
            ...
            <div style={{ fontSize: 13, color: C.muted, marginBottom: 20 }}>This cannot be undone. Parts using this shape ID will fall back to displaying the raw ID.</div>
