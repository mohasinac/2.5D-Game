...
export default function RPGStoryEventCreatePage() {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      await setDoc(doc(db, COLLECTIONS.RPG_STORY_EVENTS, docId), data);
      toast.success("Story event created");
      navigate("/admin/rpg/story-events");
    } catch (e) { console.error(e); toast.error("Failed to create story event"); }
  ...
  return (
    <div className="p-6 w-full max-w-3xl">
      <div className="mb-5">
        ...
        <h1 className="text-[22px] font-bold text-theme-text mt-2">Create Story Event</h1>
      </div>
      <div className={CARD + " space-y-4"}>
        ...
        <button onClick={handleSave} disabled={saving} className={BTN_PRIMARY}>{saving ? "Saving..." : "Create Story Event"}</button>
      </div>
    </div>
  );
}
