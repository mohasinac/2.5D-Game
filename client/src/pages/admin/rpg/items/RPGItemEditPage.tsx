...
export default function RPGItemEditPage() {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const snap = await getDoc(doc(db, COLLECTIONS.RPG_ITEMS, id));
        if (!snap.exists()) { toast.error("Item not found"); return; }
        const d = snap.data();
        setDisplayName(d.displayName ?? "");
        setDescription(d.description ?? "");
        setCategory(d.category ?? "consumable");
        setIconAssetId(d.iconAssetId ?? "");
        setStackable(d.stackable ?? true);
        setMaxStack(d.maxStack ?? 99);
        setUsable(d.usable ?? false);
        setUseEffectType(d.useEffect?.type ?? "none");
        setUseEffectValue(d.useEffect?.value ?? 0);
        setUseEffectEventId(d.useEffect?.eventId ?? "");
        setUseEffectMapId(d.useEffect?.mapId ?? "");
        setSellPrice(d.sellPrice ?? 0);
        setBuyPrice(d.buyPrice ?? 0);
        setQuestRelated(d.questRelated ?? false);
      } catch (e) { console.error(e); toast.error("Failed to load item"); }
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      await setDoc(doc(db, COLLECTIONS.RPG_ITEMS, id), data);
      toast.success("Item saved");
      navigate("/admin/rpg/items");
    } catch (e) { console.error(e); toast.error("Failed to save item"); }
  ...
  return (
    <div className="p-6 w-full max-w-3xl">
      <div className="mb-5">
        ...
        <h1 className="text-[22px] font-bold text-white mt-2">Edit Item: {id}</h1>
      </div>
