// ArenaSystemSideView — PixiJS cross-section profile.
...
export function ArenaSystemSideView({ arenaSystem }: Props) {
  ...
  const rebuildScene = useCallback(() => {
    ...
    axes.moveTo(startX, startY).lineTo(startX + graphWidth, startY);
    axes.moveTo(startX, startY).lineTo(startX, startY - graphHeight);
    ...
    if (elevType === "flat") {
      ...
      for (let i = 0; i <= 20; i++) {
        ...
        const ht   = 40 * Math.cos((dist / maxDist) * Math.PI / 2);
        points.push({ x: dist, y: -ht });
      }
    } else if (elevType === "pyramid") {
      points.push({ x: 0, y: -50 }, { x: maxDist, y: 0 });
    } else if (elevType === "ramp") {
      const tiltAngle = as.elevationMap.tiltAngle ?? 15;
    ...
    if (points.length > 0) {
      ...
      floorLine.moveTo(
        startX + (points[0].x / maxDist) * graphWidth,
        startY + (points[0].y / maxHeight) * graphHeight,
      );
      for (let i = 1; i < points.length; i++) {
        floorLine.lineTo(
          startX + (points[i].x / maxDist) * graphWidth,
          startY + (points[i].y / maxHeight) * graphHeight,
        );
      }
    ...
    // Wall height bar
    const wallHeight  = (as.wallProfile.baseHeight ?? 100) / maxHeight * graphHeight;
    ...
    distLabel.position.set(startX, h - 20);
    app.stage.addChild(distLabel);

    const htLabel = new PIXI.Text({
      text:  "Height ↑",
      style: { fontSize: 12, fill: 0x94a3b8 },
    });
    htLabel.position.set(10, startY - graphHeight - 10);
    app.stage.addChild(htLabel);
  }, []);
  ...
  useEffect(() => {
    ...
    void (async () => {
      ...
      if (cancelled || !containerRef.current) { try { app.destroy(true, { children: true }); } catch { /* ok */ } return; }
