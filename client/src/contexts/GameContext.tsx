// [GAME-CONTEXT] GameContext — global game settings state.
// Migrated from Next.js contexts/GameContext.tsx — "use client" removed (Vite is always client).
...
export function GameProvider({ children }: { children: React.ReactNode }) {
  ...
  const resetGame = useCallback(() => {
    setSettings((prev) => ({ ...defaultSettings, userId: prev.userId, username: prev.username }));
  }, []);
  ...
  const isReady = Boolean(settings.beybladeId && settings.arenaId && settings.gameMode);
