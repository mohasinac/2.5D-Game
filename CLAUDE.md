...
- **SpawnManager octagon base fallback corrected to 0**: `_resolveSurface` octagon fallback was erroneously set to `DEFAULT_BASE_HEIGHT` (30) — code audit confirmed octagon top is at world Y=0 (`baseMesh.position.y=-H/2`, `buildTopFaceGeo` called with `baseHeight=0`). Fixed: `bestSurfY = 0`.
- **SpawnManager wall rimY corrected to `arena.posY`**: `_resolveWalls` rimY was erroneously set to `DEFAULT_BASE_HEIGHT + arena.posY` (= 30). The actual arena wall mesh starts at `rimY=arena.posY=0` per `_arenaRimY()` (same fix that corrected floating walls 2026-06-05). SpawnManager wall-height cull must match: fixed to `rimY = arena.posY`.
...
- **`src/features/managers/SpawnManager.ts`** (new): Physics probe — a pyriform spinning-top entity that can be spawned into the Arena Sandbox to interactively test geometry, speed lines, traps, zones, and wall collisions.
  ...
  - **Bowl surface resolution** (`_resolveSurface`): iterates all arenas via `worldToArenaLocal` + `arenaSurfaceYAtArenaLocal`; tip-contact (not sphere); `MAT_RESTITUTION` per `arena.baseMaterial`; octagon base fallback at world Y=0 (octagon top face is at Y=0, not Y=30).
...
- **Bug fixes in SpawnManager (found during audit — session 2)**:
  - `_applyTraps` base trap `surfY` must match actual trap mesh Y. `trapSurfY(trap, arenas, baseHeight)` returns `baseHeight = 30` for base traps — base trap meshes ARE at world Y≈30 (pre-existing system-wide bug: base traps float 30cm above octagon surface at Y=0). SpawnManager uses `surfY = DEFAULT_BASE_HEIGHT = 30` to match the mesh location; base traps only trigger when ball jumps to ≈Y=40 (tip at Y=30).
...
- **Octagon top face is at world Y=0, not Y=30**: `DEFAULT_BASE_HEIGHT = 30` is the HEIGHT (thickness) of the octagon platform, not its world Y. Code: `baseMesh.position.y = -H/2 = -15` puts top at 0; `buildTopFaceGeo(..., baseHeight=0, ...)` puts top face at Y=0. Arena mesh: `baseY=0`, `mesh.position.y=posY=0` → rim at Y=0. `arenaSurfaceYAtArenaLocal` uses `H=arena.posY=0` → returns 0 at rim. `_resolveSurface` octagon fallback must be `bestSurfY = 0`.
...
- **Base trap `surfY` must match trap mesh Y (= `DEFAULT_BASE_HEIGHT`)**: `trapSurfY(trap, arenas, baseHeight)` returns `baseHeight=30` for base traps; trap meshes ARE at world Y≈30 (pre-existing system-wide bug: `baseMesh.position.y=-H/2` puts octagon top at Y=0 but `trapSurfY` uses `baseHeight=30`). SpawnManager must use the same `surfY=DEFAULT_BASE_HEIGHT` so the proximity check `|(tipY − surfY)| ≤ 4` triggers when ball is at the correct height to overlap the floating trap mesh.
...
- **`ArenaEnvironmentManager` interval timer init needs `Math.max(0, ...)` in BOTH `tick()` and `addEntry()`**: `entry._timer = entry.intervalSec - entry.delaySec` can produce a negative value when `delaySec > intervalSec`. The clamp must appear in both the `tick()` initial assignment and `addEntry()` (which bypasses `tick()`'s init path). Always use `Math.max(0, entry.intervalSec - entry.delaySec)` in both.
...
- **`_envMgr` must be instantiated before `loadArena()` in `buildCustom()`**: `_envMgr` is declared with `!` non-null assertion. Any code path during `loadArena()` that calls `this._envMgr.method()` without optional chaining throws at runtime. Order: `_envMgr = new ArenaEnvironmentManager(...)` → then `this.loadArena()`. The earthquake/RPM env-trigger paths use `this._envMgr.triggerEvent(...)` without `?.`.
- **Score HUD condition must be `pointsPerSecond > 0` only**: The OR condition `pointsPerSecond > 0 || scoreMultiplier !== 1` shows a score row for arenas with a non-default multiplier but zero passive points — the score never changes, showing a static `0 pts`. Use `pointsPerSecond > 0` alone.
- **`_onEnvChange` weather surface-map path must call `updateArenaChildren()` + `rebuildDependentsOf()`**: After `applyArena()` + `_applyArenaTilt()`, rim walls and anchored bridges are stale unless `this.updateArenaChildren(arena)` and `this.rebuildDependentsOf(arenaId)` are called. Without them, wall and bridge geometry drifts out of sync with the arena surface after a weather-triggered surface swap.
- **SpawnManager `_currentGravityScale` tracks active arena gravity**: `_applyGravity()` uses a fixed `GRAVITY` constant. `_resolveSurface()` must update `this._currentGravityScale = arena.gravityScale ?? 1.0` whenever a better surface match is found. `_applyGravity()` must multiply: `GRAVITY * this._currentGravityScale * dt`. Without this, the gravity scale env slider has zero effect on the physics test ball.
