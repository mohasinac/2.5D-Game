...
export class ArenaSandbox extends Sandbox {
  ...
  constructor(container: HTMLElement, opts: SandboxOptions) {
    ...
    this.sceneTree.onDelete = (ids)=>{
      for (const id of ids) {
        ...
        if(arena){
          // Remove all pits and zones belonging to this arena
  ...
  private renderProps(): void {
    ...
    if(arena){
      this.props.showArena(
        arena,
        ()=>{ applyArena(arena,this.getArenaHoles(arena)); this.updateArenaFloor(arena); this.updateIslandCap(arena,id); this.updateAllMoatIslandCaps(); this.updateTopFace(); this.saveArena(); },
        ()=>{ applyArena(arena,this.getArenaHoles(arena)); this.updateArenaFloor(arena); this.updateIslandCap(arena,id); this.updateAllMoatIslandCaps(); this.updateTopFace(); this.renderProps(); this.saveArena(); },
    ...
    if(pit){
      ...
      this.props.showPit(
        pit, parentArena,
        ()=>{ applyPit(pit,parentArena,this.pits,this.zones); this.updateArenaBowlHoles(parentArena,pit.parentArenaId); this.updateArenaFloor(parentArena); this.checkSiblingConflicts(this.directParentId(pit),this.directParentType(pit)); this.saveArena(); },
        ()=>{ applyPit(pit,parentArena,this.pits,this.zones); this.updateArenaBowlHoles(parentArena,pit.parentArenaId); this.updateArenaFloor(parentArena); this.checkSiblingConflicts(this.directParentId(pit),this.directParentType(pit)); this.renderProps(); this.saveArena(); },
        (name)=>{ this.sceneTree.setLabel(id,name); this.saveArena(); },
        ()=>{ const ec=new THREE.Color(pit.color).lerp(new THREE.Color(0xffffff),0.5);(pit.edges.material as THREE.LineBasicMaterial).color.copy(ec); applyPit(pit,parentArena,this.pits,this.zones); this.saveArena(); },
        ()=>{ applyPit(pit,parentArena,this.pits,this.zones); this.saveArena(); },
      );
      return;
    }
    ...
    if(zone){
      ...
      this.props.showZone(
        zone, parentArena,
        ()=>{ applyZone(zone,parentArena,this.getScene(),this.pits,this.zones); this.updateArenaBowlHoles(parentArena,zone.parentArenaId); this.updateArenaFloor(parentArena); this.checkSiblingConflicts(this.directParentId(zone),this.directParentType(zone)); this.saveArena(); },
        ()=>{ applyZone(zone,parentArena,this.getScene(),this.pits,this.zones); this.updateArenaBowlHoles(parentArena,zone.parentArenaId); this.updateArenaFloor(parentArena); this.checkSiblingConflicts(this.directParentId(zone),this.directParentType(zone)); this.renderProps(); this.saveArena(); },
        (name)=>{ this.sceneTree.setLabel(id,name); this.saveArena(); },
        ()=>{ applyZone(zone,parentArena,this.getScene(),this.pits,this.zones); this.saveArena(); },
      );
      return;
    }
  ...
  /** Get the direct parent id of a pit or zone (arena, pit, or zone). */
  ...
  /** Collect sibling pits/zones that share the same direct parent. */
  ...
  /**
   * Check sibling conflicts. Partial overlap between pit+zone is forbidden (red warning).
   * Same-type partial overlap → merge.
   */
  private checkSiblingConflicts(parentId: string, parentType: 'arena' | 'pit' | 'zone'): void {
    ...
    // Build a set of IDs that currently conflict so we can clear resolved ones
    ...
    for (let i = 0; i < siblings.length; i++) {
      for (let j = i + 1; j < siblings.length; j++) {
        ...
        const overlapping   = dist < sumR;   // any overlap
        const contained     = dist <= diffR; // one fully inside the other

        if (!overlapping) continue; // no overlap at all, skip

        if (a.kind !== b.kind) {
          // pit+zone partial overlap (but NOT containment) is forbidden
          if (!contained) {
            newConflicts.add(a.id); newConflicts.add(b.id);
          }
        } else {
          // same-type overlap → merge
          if (!contained) {
            this.mergeSameType(a, b, parentId, parentType);
            // after merge these IDs are gone, stop this pass
            return;
          }
        }
      }
  ...
  /** Convex hull (Andrew's monotone chain) of 2D points. Returns hull in CCW order. */
  ...
  /** Merge two same-type siblings into one combined scoop. */
  private mergeSameType(
    ...
    // Collect all outline points for both items in arena-local XZ
    ...
    // Compute merged centre as centroid of hull
    ...
    if (a.kind === 'pit') {
      ...
      // Build geometry with convex hull shape
      ...
      const [mesh, edges, fillMesh, fillLight] = buildZoneObjects(mergedZone, arena, this.pits, this.zones);
      mergedZone.mesh = mesh; mergedZone.edges = edges; mergedZone.fillMesh = fillMesh; mergedZone.fillLight = fillLight;
      ...
      this.sceneObjects.set(newId, fillLight ? [mesh, edges, fillMesh, fillLight] : [mesh, edges, fillMesh]);
  ...
  /** Register a newly created child into its parent's pitIds or zoneIds list. */
  ...
  /** Remove a pit from scene + data (does NOT remove from parent list — caller must do that). */
  ...
  /* ── Add pit to arena (original, preserved) ── */
  ...
  private addPitToParent(parentId: string, parentType: 'arena' | 'pit' | 'zone'): void {
    ...
    // Also add to root arena's pitIds for global tracking
  ...
  /* ── Generic add zone to any parent ── */
  private addZoneToParent(parentId: string, parentType: 'arena' | 'pit' | 'zone'): void {
    ...
    const [mesh, edges, fillMesh, fillLight] = buildZoneObjects(zone, arena, this.pits, this.zones);
    ...
    // Also add to root arena's zoneIds for global tracking
  ...
  /** Recursively restore a saved pit and its nested children into the scene. */
  ...
  /** Recursively restore a saved zone and its nested children into the scene. */
  private restoreZoneSave(zs: ZoneSave, arenaId: string, parentId: string, data: ArenaData): void {
    ...
    const[zm,ze,zfm,zfl]=buildZoneObjects(zone,data,this.pits,this.zones);
  ...
  private loadArena(): void {
    try {
      ...
      for(const as of cfg.arenas){
        ...
        // Restore pits and zones (recursively including nested children)
