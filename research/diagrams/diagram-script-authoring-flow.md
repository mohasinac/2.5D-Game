...
flowchart TD
  ...
  subgraph "Script Definition Types"
    ...
    SD_COMBO[ComboDoc<br/>sequence[3], cost, type, effectId?<br/>⚠️ effectId missing in current schema]
  end
  ...
  subgraph "Compilation (save-time)"
    ...
    CC_MULT[multiplier → factor.boost BehaviorRef]
  ...
  subgraph "Engine Runtime"
    ...
    ER_DISPATCH[executeBehavior()<br/>⚠️ only movement.orbit currently wired]
...
|-----------------|--------------------------|---------------|
| `multiplier` | `factor.boost` / `factor.defense` | ❌ No |
| `transformation` | `transform.become_X` | ❌ No |
| `spawning` | `spawn.portal` / `spawn.projectile` | ❌ No |
| `movement` | `movement.orbit` / `movement.circle` / `movement.dash` | ⚠️ orbit only |
| `arena_effect` | `arena.floor_override` / `arena.add_hazard` | ❌ No |

## Critical Missing Wire

`ComboEffectDef` documents are compiled and stored in Firestore correctly. The `executeBehavior()` runtime dispatcher in `ArenaFeatureProcessor` only handles `movement.orbit`. All other BehaviorRef types need dispatch cases wired in Stage X (BehaviorRef Dispatch implementation stage).
